...
var mostCommonWord = function(paragraph, banned) {
    // todo: maybe there's a better way to do this regex
    let replaceAll = paragraph.replace(/\!/gi, ' ')
                              .replace(/\?/gi, ' ')
                              .replace(/\,/gi, ' ')
                              .replace(/\;/gi, ' ')
                              .replace(/\./gi, ' ')
    ...
    for (let i = 0; i < words.length; i++) {
        ...
        if (!bannedWordSet.has(lowerWord)) {
...
let banned = ["m","q","e","l","c","i","z","j","g","t","w","v","h","p","d","b","a","r","x","n"];
